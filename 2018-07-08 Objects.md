...
A shallow copy is fairly understandable and has far less issues. ES6 has introduced `Object.assign(..)` for this task. `Object.assign()` takes a _target_ object as its first paramter and one ore more _source_ objects as its subsequent paramteres. It iterates over all the _enumerable_, _owned keys_ on the _source_ objects and copies them (via `=` assignment only) to _target. For example:
...
The ability for you to change the value of a property is controlled by `writable`. Trying to change a property with `writable: false` is silently fail or return a `TypeError` in `strict mode`.
...
Changing `configurable` to `true` will result in a `TypeError` regardless of `strict mode`. Changing `configurable` to `false` is a one-way action and cannot be undone.
...
Property accesses are performed with a default built-in `[[Get]]` operation  (like a function call on the object). The `[[Get]]` operation first inspects the object for a property of the requested name, and if it finds it, it will return the value acordingly. If it cannot find the value, it returns the value `undefined`.
...
If the property is not present the `[[Put]]` operation is even more complex. We will discuss then in Chapter 5 when discussing `[[Prototype]]`.
...
You almost always want to declare both a getter and a setter.
